...
def parse_text(text):
    ...
    def _has_nonascii(x):
        return bool(re.search(r"[^\x00-\x7F]", x or ""))

    def _is_clean_numeric_date(x):
        x = normalize(x or "")
        return bool(
            re.fullmatch(r"\d{2}/\d{2}/\d{4}", x) or
            re.fullmatch(r"\d{4}/\d{2}/\d{2}", x)
        )

    def _assign_pair(a, b):
        if is_gregorian_date(a):
            return a, b
        if is_gregorian_date(b):
            return b, a
        # garbled Gregorian month (non-ASCII chars) vs clean numeric Ethiopian
        a_clean = _is_clean_numeric_date(a)
        b_clean = _is_clean_numeric_date(b)
        a_garbled = _has_nonascii(a)
        b_garbled = _has_nonascii(b)
        if a_clean and b_garbled:
            return b, a  # b is garbled Gregorian, a is clean Ethiopian
        if b_clean and a_garbled:
            return a, b  # a is garbled Gregorian, b is clean Ethiopian
        # year heuristic: higher year = Gregorian (Gregorian > Ethiopian by ~7-8 yrs)
        def _year(x):
    ...
    if len(date_pairs) >= 1:
        ...
        a, b = date_pairs[0]
        dob_greg, dob_eth = _assign_pair(a, b)

    if len(date_pairs) >= 2:

        a, b = date_pairs[1]
        exp_greg, exp_eth = _assign_pair(a, b)
...
def confirm_dates(data):
    ...
    def garbled_but_acceptable(x):
        """
        Gregorian date where OCR mangled the month abbreviation into
        Amharic characters (e.g. 2034/እ48ሃ/05).  Year and day are still
        readable, so accept it instead of rejecting the whole scan.
        """
        if not x:
            return False
        if not re.search(r"[^\x00-\x7F]", x):
            return False  # not garbled at all — use normal check
        year_m = re.match(r"(\d{4})/", x)
        day_m  = re.search(r"/(\d{2})$", x)
        if year_m and day_m:
            year = int(year_m.group(1))
            day  = int(day_m.group(1))
            return 1900 <= year <= 2100 and 1 <= day <= 31
        return False
    ...
    if (suspicious(dob_greg) or not is_date(dob_greg)) and not garbled_but_acceptable(dob_greg):
    ...
    if (suspicious(exp_greg) or not is_date(exp_greg)) and not garbled_but_acceptable(exp_greg):
